...
%==========================================================================
    ...
    function cout = getCoordBox(hObj)
        this_N = hObj.N; % Get bounds
        this_dims = hObj.dims;
        guide = cell(this_dims,1);
        for ii = 1:this_dims
            % the box for this dimension
            guide{ii} = ([1:(this_N(ii)+1), this_N(ii):-1:2]-1).'; 
        end
        ...
        if this_dims>1
            ...
            cout = hObj.coord_rel(M);
        else
            cout = hObj.coord_rel(guide);
        end
    end
    ...
    function [g_ind g_coord] = allocate(hObj, s, centroid)
        ...
        for ii = 1:dims
            if (any(centroid(:,ii) <= s.lower(ii)) ... 
                    || any(centroid(:,ii) >= s.upper(ii)))
