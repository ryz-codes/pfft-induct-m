...
%% Fill RHS
b = zeros(rN*zN,1);
b(n_int) = z_step1*mu./4./pi.*(z_all(n_int)-zp)./(sqrt(r_all(n_int).^2 + (z_all(n_int)-zp).^2).^3);
b(n_int2) = mu./4./pi./sqrt(r_all(n_int2).^2 + (z_all(n_int2)-zp).^2);
